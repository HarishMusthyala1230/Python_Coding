" Python program for implementing Brute-force attack on Simplified Des"
...
def Function(After_IP,KEY,SBOX):
    ...
    round1 = L_P4 + Right
    return round1
# Program starts from here (taking known plain text and Cipher text as input from user)
Text_input = input("Enter 8 bit known Plain Text : ")
cipher_input = input("Enter 8 bit known cipher text : ")
# Storing the input in a List
for i in str(Text_input):
	Plain_Text.append(int(i))
for i in str(cipher_input):
	Cipher_Text.append(int(i))
SBOX = 0
Key = [0]*10
a = 1
for n in range(0,1024): # For all possibilities of 10  bit Keys
    #Key = [0]*10
    for i in range(9,-1,-1): # For converting decimal to 10 bit binary
        q = n%2
        Key[i] = q
        n = int(n/2)
    # Key Generation
    ...
    if Plain == Plain_Text: # If the plain text calculated matches with original loop breaks
        print("Key ",a,": ",Key)
        a = a + 1
" Program Ends "
